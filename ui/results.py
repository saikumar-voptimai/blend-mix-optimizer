"""
UI Results — Best blend card and top blends table.
"""
...
def render_best_blend_card(
    ...
    if getattr(result, "fe_constraint_relaxed", False):
        st.warning(
            ...
            f"{min_fe_production_mt:.0f} MT Fe production. "
    ...
    if fuel_result:
        with st.expander("🔥 Fuel Slag Breakdown", expanded=False):
            ...
            fe1.metric("Coke Fe", f"{fuel_result.coke_fe_mt:.2f} MT", "Fe2O3 in ash → Fe")
            fe2.metric("Nut Coke Fe", f"{fuel_result.nut_coke_fe_mt:.2f} MT", "Fe2O3 in ash → Fe")
            fe3.metric("PCI Fe", f"{fuel_result.pci_fe_mt:.2f} MT", "Fe2O3 in ash → Fe")
            fe4.metric("Total Fuel Fe", f"{fuel_result.total_fuel_fe_mt:.2f} MT")

    with st.expander("Full chemistry + composition"):
        qty_cols = st.columns(max(1, len(result.quantities)))
        for i, (ore, qty) in enumerate(result.quantities.items()):
            pct = (qty / result.total_qty * 100.0) if result.total_qty > 0 else 0.0
            qty_cols[i % len(qty_cols)].metric(ore, f"{qty:.0f} MT", f"{pct:.1f}%")
        ...
        net_fe_display = f"{net_fe_pct:.3f}%"
...
def render_top_blends_table(grid_df: pd.DataFrame, fuel_input: FuelInput = None):
    """Render the top blends comparison table sorted by cost with actual blend recipe."""
    if grid_df.empty:
    ...
    show_df = show_df[display_cols].rename(columns=rename_map).head(20).copy()
    ...
    for col in show_df.columns:
        ...
        elif "MT" in col or col in rename_map.values():
